...
帮我新增座位表功能，支持座位表展示、座位表编辑、人名搜索、随机抽人。有关数据全部需要存入本地。

1. 座位表展示：屏幕上方是教室前侧（黑板讲台），下方为教室后侧。座位表以一个一个单元格为基本单位（但是不显式显示单元格），一共有三种块：人名块，占地横向两个单元格，里面写一个人名；过道块，占地一个单元格；讲台块，占地横向两个单元格；各种块拼接在一起即可组成座位表。
2. 座位表编辑：支持手动编辑（在网页上进行拖动位置、添加、删除等操作）和从 csv 导入两种方式。csv 的格式你可以自行设计。也支持导出 csv。在网页上提供一个简单的编写该 csv 的教程。
3. 人名搜索：支持中文、拼音首字母（如，车一诺可以由 cyn 搜索到）提供一个搜索框，每输入一个字符就实时高亮座位表中所有以搜索框内内容为前缀的人名块。因此不设搜索按钮。
4. 随机抽人：按下随机抽取按钮之后，随机抽一个人，具体地，执行若干次单个人随机高亮，两次高亮时间间隔依次减小，总时间控制在 6s，最后一次高亮的人就是抽取结果，此时要有一个弹窗动画展示抽中的人。

请你合理设计 UI，做到简洁美观现代，样式和已有代码保持统一。请直接在对话框中输出要改/新增的文件的完整代码，而不是使用工作区读写。





请你再增加一个模态框定时关闭功能，对于更新日志、公告、寻物、通知模态框以及后续可能出现的新模态框，在模态框的标题栏添加 UI，允许设定一个自动关闭倒计时，时间一到自动关闭模态框。请你合理设计 UI，做到简洁美观现代，样式和已有代码保持统一。请直接在对话框中输出要改/新增的文件的完整代码，而不是使用工作区读写。

请你先告诉我，完成这个任务，你需要哪些代码？
...
[ 20] js/features/modal_timetable.js
[ 21] js/features/school_schedule.js
[ 22] js/features/store.js
[ 23] js/features/timeline.js
[ 24] js/features/weather.js